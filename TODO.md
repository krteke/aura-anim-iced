...
- `[x]` Implement keyframe segment lookup with edge handling for empty tracks, single-frame tracks, exact offsets, and between-frame sampling.
...
- `[x]` Build the `examples/keyframes_popup.rs` demo with popup opacity, scale overshoot, settle timing, and keyframe-driven effect snapshots.
- `[x]` Build the `examples/timeline_toast.rs` demo with enter, hold, exit, opacity, translate-y, and completion cleanup through the runtime.
- `[x]` Add shared example helpers for app state, runtime storage, tick messages, and minimal reusable animated style mapping.
- `[x]` Add example README snippets that show the run commands and the purpose of each v0.1 example.
