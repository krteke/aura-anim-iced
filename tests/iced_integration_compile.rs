...
fn public_prelude_drives_iced_integration_helpers() {
    ...
    let effects = tick_effect_snapshot_for(&tick, target);

    assert!(effects.opacity.is_some());
    assert!(effects.scale.is_some());
...
fn iced_integration_routes_ticks_updates_runtime_and_reports_idle_output() {
    ...
    let active_effects = tick_effect_snapshot_for(&active_tick, target);
    ...
    let idle_effects = tick_effect_snapshot_for(&idle_tick, target);
