...
- Added `MotionId` and per-playback `PlaybackId` identifiers, plus
  `play_tracked` and `transition_to_tracked` for exact event matching.
- Added `MotionRuntime::events`, `take_events`, `clear_events`, and
  `pending_event_count`; events remain queued until explicitly consumed.
- Added `MotionRuntime::command_all` for pausing, resuming, seeking, finishing,
  or canceling every stored motion with normal lifecycle event semantics.
- Added event-driven `Presence::handle_event`, which only unmounts for the
  current exit playback and ignores stale completion events.
- Added idempotent `Presence::set_visible` and `Presence::toggle` controls.
- Added typed field descriptors generated by `#[derive(Animatable)]`, including
  `PositionFields::x` constants and the `field!(Position::x)` macro.
- Added `fields().animate(...)` plans for running different Tween, Spring,
  Keyframes, Timeline, or custom animations on fields of one `Motion<T>`.
- Field animation factories receive the current sampled field value so
  interrupted playback continues without jumping, while unregistered fields
...
- Added `SpringConfig::new`, `with_mass`, and `with_epsilon` constructors for
  concise custom physical configuration.
...
- Added `AnimationExt::then` and `AnimationExt::delay` combinators for creating
  `Sequence` timelines directly from concrete animations.
